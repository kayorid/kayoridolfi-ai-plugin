...
Webhook que recebe incidentes do PagerDuty e gera **runbook automático** em `.kai/runbooks/INC-<id>.md`, pré-populado com contexto, comandos sugeridos e checklist de mitigação.
...
# → escreve .kai/runbooks/INC-PD-DEMO-001.md
```
...
`.kai/runbooks/INC-<id>.md` contém:
- Contexto do incidente (importado)
- Diagnóstico inicial sugerido
- Checklist de mitigações comuns
- Próximos passos pós-mitigação (link para `/kai-retro --post-mortem`)
...
- Não executa comandos arbitrários — apenas escreve arquivos sob `.kai/runbooks/`.
