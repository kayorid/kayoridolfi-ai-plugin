...
// Kayoridolfi AI Slack bot — Bolt JS.
// Mock mode: KAI_MOCK_SLACK=1 (não conecta ao Slack; usado em testes E2E).

import { registerRetroHandler } from './handlers/retro.js';
import { registerHelpHandler } from './handlers/help.js';

const MOCK = process.env.KAI_MOCK_SLACK === '1';
...
async function buildApp() {
  if (MOCK) {
    ...
    const fakeApp = {
      ...
      async start() {
        console.log('[kai-slack] mock mode — bot pronto sem conectar a Slack');
        return { handlers };
      },
      _handlers: handlers,
    };
...
async function main() {
  ...
  if (!MOCK) console.log(`[kai-slack] bot rodando em :${port}`);
}
