...
```bash
KAI_MOCK_JIRA=1 bash integrations/jira/adapter.sh JIRA-DEMO
```
...
export JIRA_USER=devops@example.com
...
Consumido pelo comando `/kai-spec-from-ticket` para gerar `docs/specs/_active/<slug>/requirements.md`.

## Segurança

- Token nunca commitado (`pre-write-guard.sh` bloqueia).
- Adapter não envia dados a terceiros — só lê do Jira.
- Em produção , rodar via cofre de credenciais da Plataforma.
