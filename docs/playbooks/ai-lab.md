# Kayoridolfi AI Lab — Workshop hands-on quinzenal
...
**Sequência:**
1. `bash plugins/kai-sdd/scripts/init-spec.sh minha-feature`
2. Preencher `requirements.md` com 3 user stories + 5 critérios EARS
3. Rodar `/kai-spec-review` (gera feedback automatizado)
4. Discutir ambiguidades em pares

**Pré-requisito:** SDK instalado, plugin kai-sdd ativo.
...
2. Rodar `/kai-obs-checklist` no endpoint
3. Implementar correções; rodar `/kai-obs-slo` para definir SLO
...
2. Ler `plugins/kai-security/hooks/scripts/pre-write-guard.sh`
...
**Sequência:**
1. Rodar `/kai-cost-report` no repo do Lab
...
**Sequência:**
1. `/kai-evals-init <feature>`
2. Definir golden dataset (5-10 exemplos)
3. Escrever rubricas / asserts
4. Rodar `/kai-evals-run` e analisar resultado

### Trilha 6 — Retrospectivas que geram aprendizado

**Objetivo:** retro de feature real culminando em padrão promovido para o core (via `/kai-retro-promote`).

**Sequência:**
1. Escolher feature recentemente entregue
2. Rodar `/kai-retro` (estrutura guiada)
