# Plugin Development Guide — Kayoridolfi AI

Guia para desenvolver plugins Kayoridolfi AI (corp ou squad-custom). Inclui anatomia, convenções, padrões testados, armadilhas e checklist de release.
...
- Necessidade de namespace próprio (`/kai-<plugin>-<comando>`).

❌ **NÃO crie plugin** quando:
- Capacidade específica de um squad → use `.kai/skills/kai-<squad>-<skill>/` no repo do squad.
- Apenas 1 comando → adicione a um plugin existente.
- Experimentação curta → use `/kai-spike`.
...
{
  "name": "kai-<nome>",
  "description": "Kayoridolfi AI — <propósito em uma frase>",
  "version": "0.3.0",
  "author": {
    "name": "Kayoridolfi",
...
A `description` é **crítica** — o agente decide se invoca a skill com base nela. Skills Kayoridolfi AI usam **descrições ricas em português** com triggers explícitos.
...
Invoque a skill `kai-<plugin>` para <propósito>:
...
| **COMPLIANCE** | Bloqueia, exceção via `/kai-exception` | mcp-allowlist, audit-log |
...
```markdown
# Em plugins/kai-X/commands/foo.md

Invoque o comando do plugin kai-Y:

```bash
bash "${CLAUDE_PLUGIN_ROOT}/../kai-Y/scripts/<script>.sh"
```
```

Note `${CLAUDE_PLUGIN_ROOT}/../kai-Y` — funciona porque plugins estão em pastas vizinhas.
...
**P: Posso criar plugin que substitua `kai-ai-core`?**
R: Não. `kai-ai-core` é obrigatório e provê constitution/hooks bloqueantes inegociáveis. Estenda via novos plugins.

**P: Como testar plugin localmente antes de PR?**
R: Aponte temporariamente seu `~/.claude/settings.json` para o caminho local do repo. Rode `/kai-doctor` e teste manualmente.
...
R: Cache de "fase atual" em `.kai/_runtime/` atualizado por hook menos frequente. Curto-circuito antes de operações caras (find, grep recursivo).
