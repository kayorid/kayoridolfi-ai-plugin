# Kayoridolfi AI Plugin — Roadmap Evolutivo (v0.2 → v3.0)

> Plano detalhado de evolução do Kayoridolfi AI Plugin após a v0.1 estar em piloto. Cobre 5 frentes (Encantamento, Inteligência Operacional, Integrações, Comunidade & Gamificação, Robustez Técnica) sequenciadas em 6 ondas (v0.2 → v3.0).

**Data:** 2026-05-10
**Autor:** maintainers
**Status:** Plano · pronto para priorização e início após validação do piloto v0.1
**Documentos relacionados:**
- [Design original v0.1](./2026-05-10-kayoridolfi-ai-plugin-design.md)
...
> "Squad  onboarda em IA mais rápido que squad equivalente em qualquer outra empresa do setor. Devs  se tornam referência de mercado por *como* operam IA, não só pelo produto que entregam."
...
Cada frente tem **owner declarado** (maintainers sempre, mas com sub-owners por iniciativa) e roda **em paralelo** com as demais — exceto onde há dependência explícita (ver §10).
...
- Configurável em `.kai/config.yaml`: `theme: default`.
- Variável de ambiente `KAI_THEME` sobrescreve.
- Cada tema redefine paleta de cores e densidade de info na statusline.
- Tema `festive` usado em releases, dias especiais (data de fundação , marcos).

**Arquivos:**
```
plugins/kai-ai-core/themes/
...
**Escopo:**
- Achievements em `.kai/achievements.json` (squad-local) e `.kai/audit/achievements-corp.json` (futuro: corporativo).
- Cada achievement tem: `id`, `name`, `description`, `unlocked_at`, `criteria_evidence`.
- Conquistas iniciais propostas:

| ID | Nome | Critério |
|----|------|----------|
| `first-bootstrap` | 🌱 Primeiro voo | Squad rodou `/kai-bootstrap` |
...
| `mature-squad` | 🏆 Squad maduro | 3+ ciclos sem exceção, 2+ retros com promoção, Champion ativo 8+ semanas. Destrava `/kai-fast` |
...
- **Comando** `/kai-achievements` lista os do squad.
- **Leaderboard** opcional via `/kai-leaderboard` (compara com média  — sem ranking individual destrutivo).

**Arquivos:**
```
plugins/kai-ai-core/achievements/
...
- Mensagens contextuais por comando:
  - `/kai-bootstrap` → "✨ Mapeando o DNA do seu squad..."
  - `/kai-spec` → "📜 Construindo spec — segura como cold storage..."
  - `/kai-threat-model` → "🔍 Pensando como atacante motivado..."
  - `/kai-retro` → "🎓 Capturando aprendizado para o  de 2027..."
...
- Configurável em `.kai/config.yaml`: `sounds: { enabled: false, on: ["security_block", "achievement"] }`.
- Default: desabilitado. Squad/dev opta in.

**Esforço:** 0.5 dia.

### A.5 — Tutorial interativo `/kai-tutorial` (v0.5)

**Motivação:** ler docs ≠ aprender fazendo. Tutorial guia o dev por um ciclo completo num sandbox.

**Escopo:**
- `/kai-tutorial` cria diretório temporário `~/.kai/tutorial-sandbox/`.
- Inicializa repo git fictício de produto cripto simples ("DemoTrading").
- Guia o usuário passo a passo por:
  1. `/kai-bootstrap` (bootstrap fictício, 5min).
  2. Pequena feature ("adicionar par BTC/USDC") via `/kai-spec` completo.
  3. Hotfix simulado.
  4. Threat model de exemplo.
  5. Retro final com promoção fictícia.
- Cada passo com explicação do "por quê" + dica.
- Tempo estimado: 45-60 min.
- Ao final: certificado markdown `tutorial-completed-<dev>.md` que vira menção em `/kai-status`.
...
**Exemplos propostos:**
- Em datas especiais (datas marcantes do time), banner muda sutilmente.
- Comando secreto `/kai-konami` mostra créditos animados.
- Após 100 commits referenciando spec, próximo `/kai-status` tem agradecimento personalizado.
- Mensagem do dia 21/04 (Tiradentes) referencia "independência operacional".
- Frase inspiracional em algum ponto do manifesto.
...
- **Comandos:**
  ...
  - `/kai-evals-run <feature>` — roda eval, gera score.
  - `/kai-evals-compare <feature> <versao-a> <versao-b>` — A/B test de prompts/modelos.
  - `/kai-evals-ci <feature>` — modo CI (exit 0/1 conforme threshold).
- **Integração:** ciclo SDD para AI features ganha fase EVAL adicional entre VERIFY e REVIEW.

**Esforço:** 12 dias.

### B.3 — `kai-knowledge-graph` (v1.5)
...
**Escopo:**
- Plugin novo `kai-knowledge-graph`.
- **Mecânica:**
  - Parser que lê todos os `design.md`, `discuss.md`, `retro.md`, `THREAT-MODEL.md` do projeto.
  - Extrai entidades (decisão, padrão, fragilidade, ferramenta) e relações (precede, contradiz, refina, motiva).
  - Persiste em `.kai/knowledge-graph/{nodes,edges}.jsonl` ou GraphML para visualização.
- **Comandos:**
  - `/kai-graph-build` — reconstrói grafo (squad-local).
  - `/kai-graph-search <consulta>` — busca semântica + estrutural.
  - `/kai-graph-related <slug>` — encontra specs relacionadas.
  - `/kai-graph-export <formato>` — exporta para Mermaid / GraphML / Neo4j Cypher.
...
### B.4 — `/kai-dashboard` ASCII (v1.0)

**Motivação:** painel rápido com métricas do squad sem sair do terminal.

**Escopo:**
- Comando `/kai-dashboard` produz painel multi-painel ASCII:
  ```
  ┌─ Kayoridolfi AI Plugin · Squad <nome> · Sem 19/2026 ─────────────────────┐
  │                                                              │
  ...
  │                                                              │
  │  Próxima ação sugerida:  /kai-spec-verify onboarding-v2       │
  └──────────────────────────────────────────────────────────────┘
  ```
- Dados extraídos de `.kai/audit/*`, `docs/specs/_active`, `_archive`, `.kai/achievements.json`, `.kai/audit/cost.log`.
- Sparklines via `gnuplot -p` minimal ou aproximação ASCII.

**Esforço:** 4 dias.

### B.5 — `/kai-search` busca semântica em specs (v1.5)

**Motivação:** após dezenas de specs arquivadas, encontrar "como tratamos rate-limit antes" requer ferramenta.

**Escopo:**
- Indexação local: walk em `docs/specs/_archive`, embeddings via modelo aprovado, sqlite-vec ou similar.
- `/kai-search "<query>"` retorna top-N specs com excerpts relevantes.
...
**Escopo:**
- Bot Slack no workspace.
- Comandos:
  - `/kai-status @squad` — status do squad em mensagem rica.
  - `/kai-help` — botão para abrir docs.
  - Notificações automáticas:
    - Novo ciclo SDD aberto em squad-relevante.
    - Hook bloqueante de SEGURANÇA disparou (canal `#kai-ai-security`).
    - Achievement destravado.
    - Exceção aberta aguardando review dos maintainers.
    - Release do SDK.
...
**Escopo:**
- Workflow `kai-ai-checks.yml` distribuído pelo SDK.
- Roda em todo PR:
...
**Escopo:**
- `/kai-spec-from-ticket <ticket-url-or-id>` — lê ticket via API, extrai descrição/critérios, gera scaffold de `discuss.md` + `requirements.md` rascunho.
- Suporta Jira (comunidade usa) e Linear.
...
- Após resolução do incidente, sugere automaticamente `/kai-runbook-from-incident` no canal Slack.
...
- Quick actions: `/kai-spec`, `/kai-status`, abrir spec ativa.
...
- Ata gerada por agente, distribuída no canal `#kai-ai-plugin`.
- Próximos encontros agendados em série recorrente no calendário.

**Esforço:** ongoing, 4h/mês do maintainers.
...
- Geração automática mensal a partir de:
  - `kai-retro-quarterly` (consolidado).
  - Achievements destravados.
  - Releases do SDK.
  - Métricas agregadas anônimas (squads ativos, ciclos, etc).
  - 1 destaque editorial (escolhido pelo maintainers).
...
**Escopo:**
- `/kai-leaderboard` mostra (sem nomes pessoais, só squad):
...
### D.5 — Workshop quinzenal "Kayoridolfi AI Lab" (v1.5)
...
- Aberto a todos os devs, não só Champions.

**Esforço:** 4h quinzenal do maintainers + convidados.
...
**Escopo:**
- 24h hackathon "Kayoridolfi AI Hack".
- Times constroem features usando o SDK.
- Categorias:
  - Melhor uso de SDD.
  - Melhor skill custom.
  - Melhor uso de kai-evals.
  - Melhor experiência de retro.
- Premiação simbólica + apresentação para liderança.

**Esforço:** evento, ~30 dias de preparo do maintainers + parceiros.
...
- Repo público `kayorid/sdd-toolkit` ou similar.
- Branding: "Inspirado pelo Kayoridolfi AI Plugin".
...
- Verifica:
  ...
  - Marketplace  resolvido corretamente.
  - Plugins  ativados conforme `enabledPlugins`.
  - Hooks executáveis e sintaxe shell válida.
  - Permissões de escrita em `.kai/`.
...
### E.2 — Snapshot/rollback de `.kai/` (v0.5)

**Motivação:** operações destrutivas em `.kai/` (re-bootstrap, migração major) podem perder contexto. Snapshot reversível mitiga.

**Escopo:**
- `/kai-snapshot create [tag]` — tar.gz de `.kai/` em `.kai/_snapshots/<timestamp>.tar.gz`.
- `/kai-snapshot list` — lista snapshots.
- `/kai-snapshot restore <timestamp>` — reverte (com confirmação).
- Snapshot automático antes de `kai-bootstrap-rescan`, migração major do SDK.
...
- Mostra mensagem: "Nova versão do SDK disponível: v0.2 → v0.3. Rode `/kai-update` para atualizar."
- `/kai-update` lê changelog, mostra breaking changes, pergunta confirmação, aplica.
...
**Motivação:** maintainers precisa de dados para decidir. Hoje só tem Slack e impressões.

**Escopo:**
- Opt-in explícito por squad em `.kai/config.yaml`: `telemetry: { enabled: true }`.
- Eventos coletados (hash, sem PII):
  - Comando invocado (`kai-spec-design`, etc).
  - Hook fire bloqueante (sem conteúdo).
  - Achievement destravado.
  - Erro em script.
- Destino: endpoint interno; retenção 90d; agregação para dashboards.
...
**Motivação:** audit logs hoje vivem em `.kai/audit/*` no repo. Para incidentes, time de SecOps precisa correlacionar com outros sinais.
...
**Esforço:** 8 dias (depende do SIEM escolhido).
...
**Escopo:**
- `/kai-sdk-release <version>` (uso interno do maintainers).
...
| 4 | `/kai-explain <hook>` mostra por que um hook bloqueou | A | v0.5 |
| 5 | `/kai-history` linha do tempo de eventos do squad | B | v1.5 |
| 6 | `/kai-onboard <dev>` gera plano de onboarding personalizado | D | v1.5 |
| 7 | Suporte a hooks escritos em Python ou TypeScript | E | v2.0 |
| 8 | Pre-built CI templates para 5 stacks comuns no  | C | v1.0 |
| 9 | `/kai-changelog` mostra mudanças do SDK com filtro | A | v0.5 |
| 10 | `/kai-feedback` formulário rápido para maintainers | D | v0.5 |
| 11 | Suporte a "modo offline" (sem GitHub) para emergências | E | v1.5 |
| 12 | `/kai-export <feature>` empacota spec completa para handover | A | v1.0 |
| 13 | Templates de spec por tipo (API, UI, batch job, ML feature) | A | v1.5 |
| 14 | `kai-ai-bootstrap` como CLI standalone (sem Claude Code) | E | v3.0 |
| 15 | Suporte a múltiplas constituições (squad pode estender corp) | A | v2.0 |
| 16 | `/kai-explain-this <arquivo>` invoca subagente especialista | A | v1.5 |
| 17 | Hot-reload de hooks sem reiniciar sessão | E | v2.0 |
| 18 | Theming por horário do dia (modo noite mais sutil) | A | v1.5 |
| 19 | `/kai-dry-run <comando>` mostra o que aconteceria | A | v1.0 |
| 20 | Métricas de "saúde" do `.kai/CLAUDE.md` (idade, completude) | E | v1.5 |
...
**Caminho crítico para v2.0:** B.2 (evals) é o item de maior valor estratégico — destrava capacidade do  construir features de IA com qualidade auditável.
...
| Frente | Esforço total estimado | Capacity maintainers |
|--------|------------------------|---------------------|
...
| Cenário | maintainers FTE | Tempo até v2.0 | Risco |
|---------|:--------------:|:--------------:|-------|
...
- `/kai-doctor` rodando em todos os squads sem erros não-tratados.
...
- Squad piloto destravou modo `/kai-fast`.

### Saída da Onda 4 (v1.5)
- 1+ feature  usando IA em runtime construída via `kai-evals`.
...
- Programa de Champions auto-suficiente (maintainers <30% do tempo).
...
| 2 | maintainers satura, vira gargalo | Alta | Alto | Programa de Champions cedo; modelo de PR distribuído |
| 3 | Integração C.5 (Datadog) não evolui por mudança de stack do  | Média | Médio | Manter B.4 (dashboard interno) como fallback |
...
| 6 | kai-evals (B.2) é over-engineered para necessidade real | Média | Médio | Validar com squad piloto que de fato esteja construindo AI feature |
...
1. **Aprovar capacity plan recomendado** (2 FTEs no maintainers) ou negociar trade-offs com roadmap.
...
3. Revisão trimestral deste roadmap pela comunidade de Champions + maintainers.
