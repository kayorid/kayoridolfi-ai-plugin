# Manual Técnico — Kayoridolfi AI Plugin
...
**Versão:** 0.1.0
**Data:** 2026-05-10
**Audiência:** maintainers, AI Champions, Tech Leads, devs interessados em entender o SDK em profundidade.
...
4. [Anatomia de um plugin](#4-anatomia-de-um-plugin)
...
**Operacionalização no Kayoridolfi AI Plugin:** a fase VERIFY do ciclo SDD exige `verification.md` mapeando cada critério EARS a uma evidência (teste rodado, observação manual, métrica). Sem PASS em todos os critérios obrigatórios, SHIP é bloqueado.
...
**Categorização do Kayoridolfi AI Plugin:**
- **SEGURANÇA:** sempre bloqueia, sem exceção via flag (ex: chave privada).
- **COMPLIANCE:** sempre bloqueia, exceção apenas via processo formal `/kai-exception`.
...
**Aplicação no Kayoridolfi AI Plugin:** os plugins `kai-review`, `kai-security` e `kai-retro` definem subagentes especializados (`kai-code-reviewer`, `kai-threat-modeler`, `kai-retro-facilitator`) que são invocados pelo agente principal quando o domínio especializado é necessário.

**Padrão emergente em 2026:** "agent orchestration" via MCPs e tool use, com agentes coordenadores que decidem quando delegar. O Kayoridolfi AI Plugin adota a versão minimalista: subagentes como skills com domínio claro, invocados pelo agente principal sob demanda.
...
**Aplicação no SDK:** constitution, `.kai/CLAUDE.md`, glossário e referências de skills são bons candidatos a cache (mudam pouco, são lidos muito). O maintainers deve orientar squads a estruturar prompts longos para maximizar hit rate.
...
**No Kayoridolfi AI Plugin:**
- **Específico do squad:** `.kai/CLAUDE.md`, glossário, skills custom, runbooks (mantidos por AI Champion).
- **Corporativo:** constitution, hooks, MCP allowlist (mantidos pelo maintainers).
- **Trimestral:** `.kai/learnings/quarterly-*.md` agregando padrões.
...
- `.kai/audit/approvals.log` registra "quem aprovou o quê quando".
- `.kai/audit/exceptions.log` registra desvios conscientes.
- `.kai/audit/hook-fires.log` registra decisões de bloqueio.
...
**Padrão:** allowlist explícita auditada pelo maintainers. Inclusão exige análise (segurança, custo, exfiltração documentada). Inspirado em práticas corporativas de gestão de dependências (Snyk, dependabot) aplicadas a tools de agente.
...
│ kai-ai-core: constitution, hooks bloqueantes, MCP allowlist       │
└─────────────────────────────────────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│ CAMADA 2 — Plugins de processo (opt-in)                          │
│ kai-bootstrap, kai-sdd, kai-review, kai-observability,               │
│ kai-security, kai-retro                                            │
└─────────────────────────────────────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│ CAMADA 3 — Contexto do squad (.kai/ no repo)                      │
│ CLAUDE.md, glossary.md, runbooks/, skills/, hooks/, audit/       │
└─────────────────────────────────────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│ CAMADA 4 — Audit trail e telemetria                              │
│ git (specs, decisões, commits) + .kai/audit/ (eventos)            │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 Fluxo de carga de uma sessão

Quando um dev abre o Claude Code num repo:

1. **Claude Code lê `~/.claude/settings.json`**: descobre marketplace `kai` e plugins habilitados.
2. **Resolve marketplace**: clona/atualiza `kayoridolfi-ai-plugin` em `~/.claude/plugins/cache/kai/`.
...
6. **Comandos ficam disponíveis**: `/kai-*` reconhecidos no input.
7. **Constitution carrega sob demanda**: skill `kai-constitution` é invocada em pontos críticos (decisão arquitetural, segurança, processo).
8. **Contexto do squad** (`.kai/CLAUDE.md`) é lido pelo Claude Code automaticamente como "project memory".

### 2.3 Interação entre plugins

Os plugins são fracamente acoplados via **convenções compartilhadas**:

- Pasta `.kai/` no repo do squad.
- Pasta `docs/specs/_active/<feature>/` para artefatos de feature.
- Audit logs em `.kai/audit/`.
- Comandos com prefixo `/kai-`.
- Hooks com payload JSON padrão.

Não há acoplamento de código — qualquer plugin pode ser desabilitado isoladamente. Mas há recomendações de combinação (ex: `kai-sdd` se beneficia de `kai-review`, `kai-security` para features críticas).
...
A descrição é **crítica**: o agente decide se invoca a skill com base nela. Skills  usam descrições em português com triggers explícitos ("acione quando o usuário disser X, Y, Z, ou estiver na fase ...").
...
Comandos  começam com `kai-` para namespacing.
...
**Convenções:**
- Nomes de skill, comando e hook usam prefixo `kai-`.
...
**Componentes:**
- `config/constitution.md` — texto carregado pela skill `kai-constitution`.
- `config/mcp-allowlist.json` — fonte de verdade para `mcp-allowlist.sh`.
- `skills/kai-constitution/SKILL.md` — skill com auto-trigger em decisões críticas.
- `commands/{kai-help,kai-status,kai-approve,kai-exception}.md` — comandos base.
...
| `destructive-confirm.sh` | `PreToolUse` | `Bash` | sim (sem flag `# KAI_CONFIRMED`) | Detecta `rm -rf`, `force-push`, `DROP`, etc |
| `mcp-allowlist.sh` | `PreToolUse` | `mcp__.*` | sim (sem allowlist) | Bloqueia MCPs fora da `mcp-allowlist.json` |
| `audit-log.sh` | `PostToolUse` | `Bash` | não | Grava `.kai/audit/commands.log` para git/k8s/terraform/aws |

**Fluxo de `/kai-approve`:**
1. Detecta spec ativa, fase, aprovador (via `git config user.email`).
2. Append em `.kai/audit/approvals.log` + `docs/specs/.../approvals.log`.
3. Commit dedicado.
...
**Componentes:**
- `skills/kai-bootstrap/SKILL.md` — orquestra fluxo de 3 passos.
- `skills/kai-bootstrap/scripts/repo-scan.sh` — análise automática (5min, sem LLM).
- `skills/kai-bootstrap/references/interview-questions.md` — 10 perguntas estruturadas.
- `skills/kai-bootstrap/assets/templates/{CLAUDE.md.tpl,glossary.md.tpl}` — templates.
...
Saída: `.kai/bootstrap/analysis.md` markdown estruturado.

**Fluxo da entrevista:**
- Skill executa as 10 perguntas uma por mensagem.
- Resposta de cada uma é appendada em `.kai/bootstrap/interview.md`.
- Após a 10ª, gera artefatos finais usando análise + entrevista.

### 5.3 kai-sdd

**Componentes:**
- `skills/kai-sdd/SKILL.md` — herança do plugin SDD original com adaptações específicas.
- `skills/kai-sdd/references/{ears-notation,phase-playbook,status-tracking,...}.md` — docs aprofundadas.
- `skills/kai-sdd/assets/templates/{requirements,design,tasks,status,...}.md.tpl`.
- `skills/kai-sdd/scripts/{init_spec.sh,update_index.py,validate_spec.py}` — automações.
- `commands/{kai-spec,kai-spec-discuss,...,kai-hotfix,kai-spike}.md` — 10 comandos.
...
- `kai-spec-design` invoca `/kai-threat-model` (de `kai-security`) se ativo crítico.
- `kai-spec-design` invoca `/kai-observability-design` (de `kai-observability`) sempre.
- `kai-spec` invoca `/kai-review-pr`, `/kai-review-spec`, `/kai-review-security` na fase REVIEW.
- `kai-spec-retro` delega para `kai-retro-facilitator` (de `kai-retro`).

### 5.4 kai-review

**Componentes:**
- `skills/kai-code-reviewer/SKILL.md` — code review estruturado.
- `skills/kai-security-reviewer/SKILL.md` — OWASP + cripto.
- `skills/kai-spec-reviewer/SKILL.md` — coverage spec ↔ implementação.
...
`/kai-review-fix` aplica correções de findings BLOCK/HIGH com commit atômico por finding e marca como RESOLVED no REVIEW.md.

### 5.5 kai-observability

**Componentes:**
- `skills/kai-observability-designer/SKILL.md` — design de logs, métricas, traces, alertas, SLOs.
- `skills/kai-observability-reviewer/SKILL.md` — auditoria de instrumentação.
...
**Componentes:**
- `skills/kai-threat-modeler/SKILL.md` — STRIDE + cripto.
- `skills/kai-compliance-advisor/SKILL.md` — Bacen, CVM, LGPD, Travel Rule, PCI.
- `skills/kai-crypto-advisor/SKILL.md` — BIP, custódia, multisig.
...
Estes hooks **não aceitam exceção** via `/kai-exception` — são inegociáveis.

### 5.7 kai-retro

**Componentes:**
- `skills/kai-retro-facilitator/SKILL.md` — conduz retro de 5 dimensões.
- `skills/kai-learning-extractor/SKILL.md` — agregação trimestral.
- `commands/{kai-retro,kai-retro-promote,kai-retro-extract-skill,kai-retro-quarterly}.md`.

**Fluxo de promoção:**
- Squad gera `retro.md` por feature.
- AI Champion roda `/kai-retro-quarterly` para consolidar.
- Padrões com ≥3 ocorrências viram propostas via `/kai-retro-promote`.
- PR ao `kayoridolfi-ai-plugin` com label `proposal`.
- maintainers + 2 Champions revisam.
...
```
.kai/audit/
├── approvals.log           # /kai-approve
├── exceptions.log          # /kai-exception
├── hook-fires.log          # bloqueios
├── commands.log            # comandos shell relevantes
├── security-events.log     # eventos do kai-security
└── _archive/
...
2026-05-10T14:35:22Z | feature=onboarding-v2 | phase=DESIGN | actor=joao@example.com | reason="aprovado em sync"
```
...
- `.kai/audit/` deve ter retention policy explícita (governança).
...
**Quando usar no Kayoridolfi AI Plugin:**
...
**Exemplo no Kayoridolfi AI Plugin:** durante PLAN, despachar em paralelo:
...
`kai-sdd` mapeia naturalmente: PLAN é planning-mode, EXECUTE é execution-mode.
...
Não está ainda no SDK v0.1, mas planejado para v1.0 como sub-plugin `kai-ai-evals` (quando squads começarem a construir features de IA, não apenas usar IA para construir).
...
2. `.kai/CLAUDE.md` (pouco mutável) — segundo, cacheável.
...
### 9.8 Gradação de bloqueio (categorização)

Padrão  que tende a se popularizar:
...
Tendência emergente: cada critério EARS é também um caso de teste executável (via parsers de EARS → assertions). Ainda exploratório no  v0.1; planejado para v1.0.
...
### 12.1 Plugin `kai-cost` (v0.2.0)

Captura uso de tokens via hook `PostToolUse` com curto-circuito antes de jq se payload sem `usage` (otimização B-4). Persiste em `.kai/audit/cost.log` com snapshot de preço (M-5) para auditoria contábil. Comandos: `/kai-cost`, `/kai-cost-feature`, `/kai-cost-budget`, `/kai-cost-alert`.
...
12 conquistas catalogadas em `plugins/kai-ai-core/achievements/definitions.json`. Checker (`achievements/checker.sh`) avalia métricas do squad e atualiza `.kai/achievements.json`. Notify hook (`achievements/notify.sh`) plugado em Stop dispara banner celebrativo ao desbloquear.
...
Comandos: `/kai-evals-init`, `/kai-evals-run`, `/kai-evals-compare` (A/B), `/kai-evals-ci` (exit 0/1).

**Integração com kai-sdd:** fase VERIFY do ciclo SDD agora invoca `/kai-evals-ci` automaticamente para features AI. Score abaixo do threshold bloqueia VERIFY.

### 12.5 Comandos de produtividade (v0.2-v0.3)

- `/kai-init` — wizard de primeira instalação que configura `~/.claude/settings.json` com 9 plugins.
- `/kai-doctor` — diagnóstico estilo `brew doctor` com 7 seções de health check coloridas.
- `/kai-snapshot` — backup tar.gz reversível de `.kai/` e specs ativas; auto-snapshot em `kai-bootstrap-rescan`.
- `/kai-dashboard` — painel ASCII com sparklines de ciclos, maturidade, achievements, próxima ação contextual.
- `/kai-tutorial` — sandbox `~/.kai/tutorial-sandbox/` com repositório fictício "DemoTrading" + roteiro 45-60min.
- `/kai-update` — verifica e aplica atualizações via `git -C` (não muda cwd) com comparação semver via `sort -V` e validação de remote semver.
- `/kai-fast` — destrava modo expresso após critérios de maturidade (3+ ciclos, 0 exceções, 2+ promotions, 5+ achievements).
- `/kai-theme set/show` — gerência do tema visual.
- `/kai-search <termo>` — busca grep estruturada em specs com excerpt.
- `/kai-new-skill <slug>` — scaffolder rápido de skill custom.
- `/kai-retro-digest [N]` — resumo das últimas N retros.
- `/kai-version` — mostra versões instaladas dos 9 plugins.
- `/kai-list-plugins` — lista plugins ativados no settings.
...
- Execução real de hooks com payloads positivos E negativos:
  ...
  - destructive-confirm: `rm -rf`, `--force`, `DROP` (bloqueia); `git status`, `--force-with-lease`, `KAI_CONFIRMED` (permite)
...
- kai-evals scripts executam (regressão de v0.3)
- ASCII art completa, documentação completa, integrações presentes

CI próprio (`.github/workflows/sdk-ci.yml`) roda em todo PR ao `kayoridolfi-ai-plugin`.
