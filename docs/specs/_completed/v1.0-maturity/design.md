...
plugins/kai-sdd/
└── commands/
    └── kai-spec-from-ticket.md  (novo) → invoca integrations/jira/adapter.sh

plugins/kai-retro/
└── commands/
    └── kai-adoption-report.md   (novo) → script em scripts/adoption-report.sh
...
- **Env:** `SLACK_BOT_TOKEN`, `SLACK_SIGNING_SECRET`, `KAI_RETROS_PATH` (default `.kai/retros`)
- **Mock mode:** `KAI_MOCK_SLACK=1` → bot inicia em modo dry-run lendo fixtures, não conecta a Slack.

### Jira adapter
- **Input:** ticket key (JIRA-1234)
- **Output:** JSON com `{title, description, acceptance_criteria, stakeholders}`
- **Modo:** `KAI_MOCK_JIRA=1` lê de `integrations/jira/mock-fixtures/JIRA-1234.json`.
...
- **Output:** escreve `.kai/runbooks/INC-<id>.md` baseado em `runbook-template.md`
- **Mock:** invocar diretamente `node webhook.js --mock fixtures/inc-001.json`

### `/kai-adoption-report`
- Lê `git log` (commits `[squad:X]`), `.kai/retros/`, `.kai/achievements.json`
...
| `plugins/kai-sdd/commands/kai-spec-from-ticket.md` | **novo** |
| `plugins/kai-sdd/scripts/spec-from-ticket.sh` | **novo** |
| `plugins/kai-retro/commands/kai-adoption-report.md` | **novo** |
| `plugins/kai-retro/scripts/adoption-report.sh` | **novo** |
...
- PagerDuty webhook precisa endpoint público. **Mitigação:** doc inclui setup com cloudflared/tunnel para dev; produção fica a cargo da Plataforma.
- Certificação pode virar burocracia. **Mitigação:** critérios numéricos auto-computáveis via `kai-adoption-report --champions`.
