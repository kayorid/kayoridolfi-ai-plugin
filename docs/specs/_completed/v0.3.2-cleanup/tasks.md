...
- [ ] T1 — Criar `plugins/kai-security/hooks/scripts/pre-write-guard.sh` agregando pii + private-key + secret-scan. Critério: exit 2 quando match, exit 0 quando limpo, <200ms para 10KB. [P]
- [ ] T2 — Atualizar `plugins/kai-security/hooks/hooks.json` Write|Edit → `pre-write-guard.sh`. Critério: smoke passa. [depende T1]
- [ ] T3 — Remover Write|Edit do `plugins/kai-ai-core/hooks/hooks.json` (secret-scan duplicado), manter pré-commit. Critério: completeness check ainda passa. [depende T2]
...
- [ ] T11 — Rodar completeness + smoke + e2e; commit `feat: Kayoridolfi AI v0.3.2 — cleanup técnico + E2E`. Tag `v0.3.2`.
