...
1. Consolidação de hooks Write|Edit num único `pre-write-guard.sh` em `kai-security`.
2. Banner SessionStart com função `strip_ansi()` antes de emitir `additionalContext`.
3. Cache de achievements em `.kai/achievements.json` com `last_evaluated_at` (epoch).
...
| Arquivo | Mudança |
|---|---|
| `plugins/kai-security/hooks/scripts/pre-write-guard.sh` | **novo** — agrega pii + private-key + secret-scan |
| `plugins/kai-security/hooks/hooks.json` | Write\|Edit aponta para `pre-write-guard.sh` |
| `plugins/kai-ai-core/hooks/hooks.json` | remove Write\|Edit (secret-scan), mantém pré-commit |
| `plugins/kai-ai-core/hooks/scripts/session-start-banner.sh` | adiciona `strip_ansi` antes do `additionalContext` |
| `plugins/kai-ai-core/hooks/scripts/mcp-allowlist.sh` | log warn se `jq` falhar |
| `plugins/kai-security/hooks/scripts/private-key-scan.sh` | `head -c 102400` no input + regex ancorada |
| `plugins/kai-ai-core/achievements/checker.sh` | função `cache_get/cache_put` com TTL 300s |
...
- **Etapas:**
  ...
  5. Retro: invoca `retro.sh` e checa `.kai/retros/*.md` criado
  6. Validate: `bash tests/smoke/run.sh` dentro do dummy
- **Limpeza:** cleanup garantido via trap.

## Riscos

- Mudar `hooks.json` de kai-ai-core pode quebrar usuários já instalados em v0.3.1. **Mitigação:** documentar em CHANGELOG + MIGRATION.md, manter comportamento equivalente.
