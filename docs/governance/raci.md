...
|-------|------|----------------------|
| **maintainers** | Time corporativo dedicado | Mantém o SDK; ~20% dos seus FTEs alocados |
...
|-----------|:----------:|:---------:|:-----------:|:---:|
| Manter `kai-ai-core` (constitution, hooks, allowlist) | R/A | C | C | I |
...
| Manter `.kai/glossary.md` | I | A | R | C |
| Aprovar fases SDD (`/kai-approve`) | I | R/A | C | C |
| Abrir exceção bloqueante (`/kai-exception`) | A | R | C | I |
| Conduzir retro de feature | I | C | R | A |
| Promover learning ao core (`/kai-retro-promote`) | A | C | R | I |
...
- Proposta direta do maintainers.
...
### 3. Review
- maintainers + 2 AI Champions de squads diferentes.
- Período de comentários: 5 dias úteis.

### 4. Decisão
- Maioria simples aprova.
- maintainers tem **veto** por motivo de segurança/compliance (com justificativa pública).
...
- Anúncio em `#kai-ai-plugin` Slack.
...
- **Regras de SEGURANÇA/COMPLIANCE:** maintainers aprova. Registrado em audit-log + GitHub issue.
...
### Auditoria
- Trimestralmente, maintainers audita exceções vigentes e expiradas.
...
### Resposta
1. Tech Lead reporta no `#kai-ai-plugin` Slack.
2. maintainers investiga (P1: <2h; P2: <8h; P3: <24h).
...
Reportadas trimestralmente pelo maintainers à liderança:

| Métrica | Cálculo | Meta |
|---------|---------|------|
| Adoção | Squads com `.kai/` no repo | crescente |
| Ciclos completados | Pastas em `_archive/` | crescente |
| Hook fires bloqueantes/sem | `.kai/audit/hook-fires.log` | decrescente após semana 4 do squad |
| Exceções abertas | issues `kai-exception` | <2/squad/mês |
| Tempo de bootstrap | até `.kai/CLAUDE.md` gerado | <1h |
| PRs ao core promovidos | conta no repo | crescente |
| NPS dos squads (semestral) | survey | >40 |

Detalhes em [`docs/plans/2026-05-10-kayoridolfi-ai-plugin-design.md`](../plans/2026-05-10-kayoridolfi-ai-plugin-design.md) §13.
