# Kayoridolfi AI Champion · Programa de Certificação
...
- Sinalizar profundidade técnica em ferramental Kayoridolfi AI
...
- ≥ 5 commits taggeados ao Kayoridolfi AI em seu squad
...
**Reconhecimento**: badge `kai-apprentice` em `.kai/people.json` (opt-in).
...
- ≥ 1 PR mergeado ao kayoridolfi-ai-plugin (correção, doc ou comando novo)
- ≥ 1 retrospectiva facilitada (não apenas participada)
- ≥ 1 padrão promovido via `/kai-retro-promote`
- Demonstrar domínio dos 4 plugins core: `kai-sdd`, `kai-review`, `kai-observability`, `kai-security`

**Avaliação**: code review pareado com 2 Champions ativos + apresentação curta (15min) em reunião comunitária.

**Reconhecimento**: badge `kai-practitioner` + elegibilidade para facilitar AI Labs.
...
- ≥ 3 PRs mergeados ao kayoridolfi-ai-plugin (≥ 1 hook ou skill nova)
...
**Avaliação**: dossiê + entrevista painel com 3 Champions ativos + maintainers.

**Reconhecimento**: badge `kai-expert` + cadeira rotativa no Comitê Champion + indicação para representar a comunidade em eventos externos sobre AI eng. interna.
...
- ≥ 1 plugin novo no kayoridolfi-ai-plugin como autor principal **ou** ≥ 1 milestone de roadmap entregue como tech lead
...
**Avaliação**: nomeação pelo Comitê + ratificação por maintainers + endorso de Diretoria.

**Reconhecimento**: badge `kai-master` + co-design do roadmap anual + mentoria formal a Practitioners e Experts.
...
```bash
bash plugins/kai-retro/scripts/adoption-report.sh --period 365d --json
bash plugins/kai-retro/scripts/leaderboard.sh --period 365d
```

Métricas extraídas automaticamente:
- PRs mergeados (via `gh pr list --author <user> --state merged --repo kayorid/kayoridolfi-ai-plugin`)
