# Roteiro de Apresentação — Kayoridolfi AI Plugin

> Roteiro slide-a-slide para apresentar o Kayoridolfi AI Plugin à liderança de Engenharia, Tech Leads e squads piloto. Duração estimada: **30-40 min** (apresentação) + **15-20 min** (Q&A).
...
> Kayoridolfi · Maio 2026 · v0.1.0
>
> *maintainers*

**Falas-chave:**
> "Boa tarde. Hoje vou apresentar o Kayoridolfi AI Plugin — uma proposta para padronizar como o  constrói software com IA, garantindo segurança, auditabilidade e velocidade. Em 30 minutos quero responder três perguntas: por que precisamos disso agora, o que é exatamente, e como começamos."
...
**Falas-chave:**
> "O  começou a usar IA generativa em desenvolvimento. Cada squad está experimentando do seu jeito. Isso é normal — e é exatamente por isso que precisamos agir agora."
>
...
## Slide 3 — As restrições reais do  (3 min)

**Visual:**
- 4 caixas: Regulação (Bacen, CVM, LGPD), Cripto-específico, Stack heterogênea, Maturidade variável.

**Falas-chave:**
> "Não estamos partindo do zero abstrato. O  tem restrições reais que tornam ferramentas genéricas insuficientes:"
>
...
## Slide 4 — O que é o Kayoridolfi AI Plugin (2 min)
...
**Falas-chave:**
> "Importante: o Kayoridolfi AI Plugin **não é** uma biblioteca de runtime. Não é um framework no estilo NestJS ou FastAPI. É um harness — define **como** trabalhamos com IA."
>
> "Tecnicamente: 7 plugins do Claude Code, um marketplace interno, e um padrão de pasta `.kai/` no repositório do squad."
>
...
>
> "**Plugins opt-in:** `kai-bootstrap`, `kai-sdd`, `kai-review`, `kai-observability`, `kai-security`, `kai-retro`, `kai-cost`, `kai-evals`. Cada squad escolhe o que faz sentido. Alguns são exigidos por contexto — `kai-security` para qualquer feature tocando ativo crítico, `kai-evals` para features que usam IA em runtime."
>
> "**Contexto do squad:** uma pasta `.kai/` no repo, gerada pelo `kai-bootstrap`, com CLAUDE.md, glossário, runbooks, skills e hooks específicos do squad. Stack-specific — porque o SDK não é."
...
|--------|-----------|--------|
| **kai-ai-core** | Constitution, hooks, MCPs, achievements, doctor, dashboard | Obrigatório |
| **kai-bootstrap** | Onboarding híbrido do squad | Recomendado |
| **kai-sdd** | Ciclo Spec-Driven rígido com checkpoints | Recomendado |
| **kai-review** | Code/security/spec review formais | Opt-in |
| **kai-observability** | Design e revisão de observabilidade | Opt-in |
| **kai-security** | Threat modeling, compliance, cripto | Obrigatório se ativo crítico |
| **kai-retro** | Retros estruturadas e memória organizacional | Opt-in |
| **kai-cost** | Captura de tokens, custo por fase/feature, alertas de budget | Recomendado |
| **kai-evals** | Eval framework para features que usam IA em runtime | Obrigatório se feature AI |

**Falas-chave:**
> "Nove plugins, cada um com responsabilidade clara. Todos compartilham convenções: prefixo `kai-`, pasta `.kai/`, comandos `/kai-*`, audit trail em `.kai/audit/`. Os dois últimos — kai-cost e kai-evals — endereçam dimensões cruciais: custo de IA é decisão de engenharia, e features que usam IA em runtime exigem framework de avaliação para evitar regressão silenciosa."

---

## Slide 8 — O ciclo SDD que o SDK impõe (4 min)

**Visual:** diagrama do ciclo de 8 fases (DISCUSS → SPEC → PLAN → EXECUTE → VERIFY → REVIEW → SHIP → RETRO) com checkpoints `/kai-approve` em cada.

**Falas-chave:**
> "Toda feature não-trivial passa por 8 fases. Cada fase produz artefato versionado. Cada fase termina com checkpoint humano via `/kai-approve` — registrado no audit trail."
>
> "Por que rígido? Porque o time está aprendendo. Disciplina agora vira intuição depois. E porque cada checkpoint é um ponto de auditoria — em um projeto regulado, isso não é opcional."
>
> "Modos especiais existem mas continuam disciplinados: **`/kai-hotfix`** para urgências (pula DISCUSS/SPEC mas exige post-mortem em 48h). **`/kai-spike`** para exploração (branch descartável, gera só learning)."
>
> "Quando o squad demonstra maturidade — 3+ ciclos completos sem exceção, retros com promoção de learnings, Champion ativo — destrava `/kai-fast` para tarefas pequenas."
...
**Falas-chave:**
> "Quando um squad começa a usar o SDK, roda `/kai-bootstrap`. Três passos:"
>
...
>
> "**Passo 2 — entrevista guiada (20-30min, squad inteiro + maintainers):** 10 perguntas estruturadas. Domínio, fluxos críticos, dores, prioridades, jargão, regulação. Não substituível por código — captura o que código não revela."
>
> "**Passo 3 — geração:** combina análise + entrevista. Gera `.kai/CLAUDE.md`, glossário, esqueleto de runbooks, plano de enriquecimento das próximas 4 semanas."
>
...
**Visual:** mapeamento entre regulações (Bacen, CVM, LGPD) → onde no audit log.
...
>
> "Para a próxima auditoria externa, em vez de reconstruir contexto a partir de Slack e memória das pessoas, abrimos a pasta `.kai/audit/` e o histórico do git."
...
|-------|----------------------|
| maintainers | ~20% FTE dedicado |
| Tech Lead | ~5% do tempo do TL |
| AI Champion | ~10% (1 por squad) |
| Dev | uso no dia a dia |

**Falas-chave:**
> "Quatro papéis. maintainers é o time corporativo dedicado. Mantém o SDK, treina champions, audita. Estimamos 20% de FTE dedicado para sustentar."
>
...
>
> "**Comunidade mensal de Champions** com maintainers dissemina padrões e captura demanda."
...
>
> "Métrica de impacto: tempo de **onboarding de dev novo**. Hoje: ~2-3 semanas até primeiro PR. Meta com SDK: -30% via `.kai/CLAUDE.md` rico e ciclo SDD que ensina o domínio enquanto entrega."
...
| Hooks bloqueantes geram fricção | Categorização gradual; canal direto com maintainers |
| Vazamento de dados via prompt | MCP allowlist + hooks PII + treinamento |
| Manutenção vira gargalo no maintainers | Modelo de PR distribui carga; rotação de revisores |
...
>
> "Risco de manutenção: distribuímos via PR aberto. Qualquer squad propõe mudança; maintainers + 2 Champions revisam. Não sobrecarrega ninguém."
...
|------|------------------|
| maintainers (20% FTE × N pessoas) | depende do dimensionamento |
...
> 2. Indicar maintainers dedicado (ou reforçar o existente).
> 3. Endossar publicamente o SDK no próximo all-hands de Engenharia.
> 4. Endossar processo de proposta (PR + review dos maintainers + Champions).
...
>
> "**maintainers dedicado:** 1-2 pessoas com 20% de tempo alocado para sustentar."
>
...
3. `/kai-bootstrap` do squad (semana +2).
4. Primeira feature pelo ciclo completo (semanas +2-+4).
5. Retro com squad + maintainers + ajustes ao SDK (semana +4).
...
> "Essas ferramentas são complementares, não substitutas. O SDK roda **em cima** do Claude Code. Mas o que estamos construindo não está no produto pronto: a constitution, os hooks de PII brasileira, o threat model cripto, o audit trail compatível com Bacen. Usar só ferramenta pronta deixa essas lacunas abertas."

### "E se o Claude Code mudar e quebrar nossos plugins?"

> "Risco real, mitigado por (1) versionamento independente do SDK, (2) smoke tests rodando contra cada release do Claude Code, (3) maintainers acompanhando releases. O SDK foi desenhado contra a API estável de plugins/hooks/MCP, não contra internas."
...
> "O código gerado pelo agente operando dentro do SDK é do , mesma política aplicada a código escrito por dev. O SDK não envia código proprietário a sistemas externos não-aprovados — MCPs externos passam por avaliação de exfiltração."

### "Não vai virar burocracia?"

> "Risco real. Mitigamos por: (1) `kai-bootstrap` é única vez por squad; (2) ciclo SDD é proporcional — features triviais usam `/kai-fast` (após maturidade); (3) categorização de hooks evita bloqueio em qualidade; (4) retros mensais ajustam o que está atrapalhando. Burocracia que não se ajusta morre. Nosso compromisso é ajustar."
...
### "Quanto tempo de maintainers realmente? 20% FTE × N pode crescer."

> "Modelagem: 1 pessoa 50% no maintainers cobre v0.1. Para v0.5 com 5 squads, 2 pessoas 50%. Para v1.0 com 10+ squads, 2-3 pessoas dedicadas + comunidade de Champions absorvendo demanda. O modelo de PR distribuído é o que escala."
...
- Demo gravado (5min) do `/kai-bootstrap` rodando em repo de teste.
