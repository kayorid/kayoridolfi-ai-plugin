# Piloto Kayoridolfi AI — instalação e teste em máquina nova

> **Para o Claude Code que está lendo este arquivo:** o usuário transferiu este repo zipado da máquina pessoal para uma máquina alvo e quer iniciar o piloto. Conduza-o passo a passo, na ordem abaixo, verificando cada item. **Nunca pule fases.** Em caso de erro, diagnostique antes de prosseguir.
>
...
Assuma que o usuário extraiu em `~/dev/kayoridolfi-ai-plugin` (ou similar).

```bash
cd ~/dev/kayoridolfi-ai-plugin    # ajustar se outro path
...
// Opção B — editar manualmente ~/.claude/settings.json
{
  "plugins": {
    "marketplaces": {
      "kai-local": {
        "type": "path",
        "path": "/Users/<seu-user>/dev/kayoridolfi-ai-plugin"
      }
    }
  }
}
...
# deve listar "kayoridolfi-ai-plugin" como local
```
...
```text
/plugin install kai-ai-core@kayoridolfi-ai-plugin
/plugin install kai-bootstrap@kayoridolfi-ai-plugin
/plugin install kai-sdd@kayoridolfi-ai-plugin
/plugin install kai-review@kayoridolfi-ai-plugin
/plugin install kai-observability@kayoridolfi-ai-plugin
/plugin install kai-security@kayoridolfi-ai-plugin
/plugin install kai-retro@kayoridolfi-ai-plugin
```

**Opt-in (ligar conforme caso):**

```text
/plugin install kai-cost@kayoridolfi-ai-plugin     # se quiser rastreio de custo de IA
/plugin install kai-evals@kayoridolfi-ai-plugin    # se construir features AI runtime
```

Critério de decisão completo em `docs/plugins/opt-in-guide.md`.

**Após instalar: reinicie o Claude Code** (para o hook `SessionStart` registrar). Você verá o banner Kayoridolfi AI no início da próxima sessão.
...
```text
/kai-init      # wizard de onboarding — gera .kai/CLAUDE.md
/kai-doctor    # health check completo (9 plugins ✓)
/kai-version   # confirma 9 plugins em 1.0.0
/kai-help      # 60+ comandos por categoria
```
...
**Critério:** Write é bloqueado, mensagem `[kai-security] BLOCKED — segredo detectado` aparece. Confirme em:
```bash
cat .kai/audit/hook-fires.log
```
...
```text
/kai-spec        # gera docs/specs/_active/<slug>/requirements.md
/kai-design      # gera design.md
/kai-tasks       # gera tasks.md
/kai-implement   # executa tasks
/kai-retro       # cria retro ao final
```

**Critério:** ao final, existem `docs/specs/_active/<slug>/{requirements,design,tasks}.md` + entrada em `.kai/retros/`.

### 6.7 — Comandos novos v0.5

```text
/kai-leaderboard
/kai-newsletter 2026-Q2
```
...
```text
/kai-adoption-report --period 30d
```
...
```bash
KAI_MOCK_JIRA=1 bash ~/dev/kayoridolfi-ai-plugin/plugins/kai-sdd/scripts/spec-from-ticket.sh JIRA-DEMO
```
...
```bash
cd ~/dev/kayoridolfi-ai-plugin/integrations/pagerduty
node webhook.js --mock mock-fixtures/inc-demo.json
```

**Critério:** cria `.kai/runbooks/INC-PD-DEMO-001.md` no projeto piloto com contexto + checklist.

### 6.11 — Slack bot offline

```bash
cd ~/dev/kayoridolfi-ai-plugin/integrations/slack
npm install                            # primeira vez (~1min)
KAI_MOCK_SLACK=1 npm run dev
```

**Critério:** log `[kai-slack] mock mode — bot pronto sem conectar a Slack`. Ctrl+C para sair.

### 6.12 — Banner SessionStart

Reinicie Claude Code uma vez (`/exit` → `claude`).

**Critério:** banner laranja "Kayoridolfi AI Plugin ativo · Squad: ..." aparece no início.

---

## Fase 7 — Checklist final de aceitação

Marque conforme valida (esse mesmo checklist em formato `tasks.md` pode ser gerado por `/kai-spec`):
...
1. **Convidar mais squads** (2-3 inicialmente) — cada um roda `/kai-bootstrap` no seu repo
2. **Agendar primeiro Kayoridolfi AI Lab** — escolha 1 das 6 trilhas em `docs/playbooks/ai-lab.md`
...
| Banner não aparece | Hook `SessionStart` não registrou | `/kai-doctor` para diagnosticar |
| `kai-evals` ou `kai-cost` não aparecem em /plugin list | Não instalados (opt-in) | Veja Fase 4 |
| Smoke test falha em macOS antigo | bash 3.2 sem suporte a `${array[@]+...}` | Atualizar para macOS 12+ ou instalar bash 5 via brew |
| Hook bloqueia onde deveria permitir | Falso-positivo | `/kai-exception` ou abrir issue com fixture |
