...
### B-1: marketplace declara `kai-ai-core@0.2.0` mas o plugin.json diz `0.1.0`
**Arquivo:** `/Users/kayoridolfi/Documents/vibecoding/kayoridolfi-ai-plugin/.claude-plugin/marketplace.json:12` vs
`/Users/kayoridolfi/Documents/vibecoding/kayoridolfi-ai-plugin/plugins/kai-ai-core/.claude-plugin/plugin.json:4`
...
**Arquivo:** `plugins/kai-cost/scripts/cost-report.sh:64` (e linha 6 `set -uo pipefail`)
**Problema:** `FEAT` só é setado no caso `feature`. Linha 64 passa
`-v feat="$FEAT"` ao awk; com `-u` ativo, qualquer `/kai-cost`,
`/kai-cost-alert` ou `/kai-cost <day|week|month>` aborta no meio do
relatório (mas só **depois** de imprimir cabeçalho — pior UX,
`exit_code` continua 0 por causa do pipe).
**Como reproduzir:**
```
mkdir -p /tmp/c/.kai/audit && cd /tmp/c
echo "2026-05-09T10:00:00Z | feature=foo | phase=PLAN | tool=Bash | in_tokens=100 | out_tokens=50 | model=x | actor=t" > .kai/audit/cost.log
bash plugins/kai-cost/scripts/cost-report.sh week
...
**Arquivo:** `plugins/kai-security/hooks/scripts/pii-scan.sh:49` e
`plugins/kai-security/hooks/scripts/private-key-scan.sh:19-20`
...
echo '{"tool_name":"Write","tool_input":{"file_path":"src/x.go","content":"card 4111111111111111"}}' \
  | bash plugins/kai-security/hooks/scripts/pii-scan.sh; echo $?
# → 0 (deveria ser 2)
```
...
**Arquivo:** `plugins/kai-cost/hooks/hooks.json:5` (matcher `.*`) +
`plugins/kai-cost/hooks/scripts/cost-capture.sh`
...
3. Cachear a "feature/phase" atuais em `.kai/_runtime/current-phase` atualizado
...
**Arquivos:** `plugins/kai-ai-core/scripts/{dashboard,doctor,update}.sh`,
`plugins/kai-cost/scripts/cost-report.sh`, `tests/smoke/run.sh`
...
- `kai-cost` não é mencionado em lugar nenhum (CHANGELOG, README, playbook);
...
**Sugestão:** adicionar bloco `## [0.2.0] — 2026-05-10` listando: `kai-cost`,
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/audit-log.sh:17` (e
todos os hooks que fazem `echo ... >> .kai/audit/<x>.log`)
...
**Arquivo:** `plugins/kai-ai-core/scripts/dashboard.sh:34`
...
em `.kai/audit/*` então OK na prática).
...
**Arquivo:** `plugins/kai-ai-core/achievements/notify.sh` — nunca
referenciado em `hooks/hooks.json`. Único caller é `commands/kai-achievements.md`.
**Problema:** dashboard menciona "achievements 0/12" e definições
existem, mas o desbloqueio só é detectado se o usuário rodar
`/kai-achievements` manualmente. Roteiro de tutorial implica que
...
**Arquivo:** `plugins/kai-ai-core/scripts/doctor.sh:107`
...
**Arquivo:** `plugins/kai-ai-core/scripts/update.sh:27,58`
**Problema:**
- assume que `~/.claude/plugins/cache/kai` é git repo limpo. Se o usuário
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/secret-scan.sh:48-49`
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/session-start-banner.sh:24-27`
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/destructive-confirm.sh:20`
...
### M-1: hooks PreToolUse Write|Edit do kai-ai-core e kai-security executam em paralelo — duplicação custosa
**Arquivos:** `plugins/kai-ai-core/hooks/hooks.json:53-60` (Write|Edit →
secret-scan) + `plugins/kai-security/hooks/hooks.json` (Write|Edit →
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/statusline.sh:57-58`
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/mcp-allowlist.sh:24`
...
prefixo `kai_*` ou `internal_*` são bloqueados. OK (failsafe), mas
sem mensagem informativa.
**Sugestão:** se `jq` retornar erro, log explícito "[kai-ai-core] WARN —
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/statusline.sh:30` e
...
**Arquivo:** `plugins/kai-cost/hooks/scripts/cost-capture.sh:39`
**Problema:** `cost.log` registra só tokens, conversão para USD/BRL
acontece no relatório. Se `KAI_PRICE_IN/OUT/USD_BRL` mudar, dados
...
**Arquivo:** `plugins/kai-ai-core/achievements/checker.sh:73-74`
...
`/kai-achievements` é invocado (sync, ~1-2s total). Aceitável mas vai
ser lento em monorepos.
**Sugestão:** cachear resultado em `.kai/achievements.json` com `last_evaluated_at` e re-avaliar só se diff > 5 min.

### M-8: SessionStart hook output usa `additionalContext` — formato OK mas conteúdo tem ANSI colors
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/session-start-banner.sh:60-82`
...
**Arquivo:** `plugins/kai-ai-core/hooks/scripts/stop-farewell.sh:39`
...
**Arquivo:** `plugins/kai-security/hooks/scripts/private-key-scan.sh:20`
...
### I-3: cross-reference `/kai-threat-model` — verificado, está correto
`kai-sdd/commands/kai-spec-design.md:12` referencia `/kai-threat-model`,
que existe em `kai-security/commands/kai-threat-model.md`. OK.

### I-4: comandos sem `argument-hint` quando recebem args (kai-update, kai-banner)
`/kai-update` e `/kai-banner` não recebem args (OK omitir). `/kai-help`,
`/kai-status`, `/kai-doctor`, `/kai-dashboard`, `/kai-achievements`,
`/kai-cost-alert`, `/kai-cost`: sem args → OK. Verificado todos os com
...
### I-6: README e MANUAL não mencionam kai-cost / achievements / tutorial
