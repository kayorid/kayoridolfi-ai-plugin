#!/usr/bin/env bash
# tests/e2e/run.sh — Suite end-to-end do Kayoridolfi AI Plugin
...
printf "${BOLD}Kayoridolfi AI E2E suite${RESET} ${DIM}— ciclo completo num sandbox temporário${RESET}\n"
...
TMP=$(mktemp -d 2>/dev/null || mktemp -d -t kai-e2e)
cleanup() {
  rm -rf "$TMP"
}
trap cleanup EXIT
...
cd "$TMP"
git init -q
git config user.email "e2e@kai.test"
...
section "2/6 — Bootstrap: criar .kai/CLAUDE.md"
mkdir -p .kai docs/specs/_active
cat > .kai/CLAUDE.md <<'EOF'
...
Maintainer: e2e
EOF
[[ -f .kai/CLAUDE.md ]] && t_pass "bootstrap stub criado" || t_fail "bootstrap stub" ""

section "3/6 — Hooks PreToolUse rodando contra payloads reais"

GUARD="$REPO_ROOT/plugins/kai-security/hooks/scripts/pre-write-guard.sh"
...
export KAI_FORCE_ACHIEVEMENT_CHECK=1
export CLAUDE_PLUGIN_ROOT="$REPO_ROOT/plugins/kai-ai-core"
if bash "$REPO_ROOT/plugins/kai-ai-core/achievements/checker.sh" >/dev/null 2>&1; then
  if [[ -f .kai/achievements.json ]]; then
    UNLOCKED=$(jq -r '.unlocked | length' .kai/achievements.json 2>/dev/null || echo 0)
    LAST_EVAL=$(jq -r '.last_evaluated_at' .kai/achievements.json 2>/dev/null)
    ...
    t_warn ".kai/achievements.json não foi criado" ""
  fi
else
...
unset KAI_FORCE_ACHIEVEMENT_CHECK
T_START=$(python3 -c 'import time;print(int(time.time()*1e9))')
bash "$REPO_ROOT/plugins/kai-ai-core/achievements/checker.sh" >/dev/null 2>&1 || true
T_END=$(python3 -c 'import time;print(int(time.time()*1e9))')
DUR_MS=$(( (T_END - T_START) / 1000000 ))
...
BANNER="$REPO_ROOT/plugins/kai-ai-core/hooks/scripts/session-start-banner.sh"
export CLAUDE_PLUGIN_ROOT="$REPO_ROOT/plugins/kai-ai-core"
