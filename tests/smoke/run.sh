#!/usr/bin/env bash
# Kayoridolfi AI · Smoke Test Suite
...
printf "\n${GOLD}${BOLD}Kayoridolfi AI · Smoke Test Suite${RESET}\n${DIM}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
...
if [[ $HAVE_JQ -eq 0 ]]; then
  ...
  HOOK_SECRET="plugins/kai-ai-core/hooks/scripts/secret-scan.sh"
  HOOK_DESTR="plugins/kai-ai-core/hooks/scripts/destructive-confirm.sh"
  HOOK_PII="plugins/kai-security/hooks/scripts/pii-scan.sh"
  HOOK_KEY="plugins/kai-security/hooks/scripts/private-key-scan.sh"
...
if [[ $HAVE_JQ -eq 1 ]]; then
  ...
  assert_allow "destructive-confirm: rm -rf com KAI_CONFIRMED" "$HOOK_DESTR" \
    '{"tool_input":{"command":"rm -rf /tmp/foo # KAI_CONFIRMED"}}'
...
TMPDIR_TEST=$(mktemp -d 2>/dev/null || mktemp -d -t kai)
mkdir -p "$TMPDIR_TEST/.kai/audit"
echo "2026-05-09T10:00:00Z | feature=foo | phase=PLAN | tool=Bash | in_tokens=100 | out_tokens=50 | model=claude | actor=test@kai | price_in=3.00 | price_out=15.00 | rate=5.30" \
  > "$TMPDIR_TEST/.kai/audit/cost.log"

(
  cd "$TMPDIR_TEST"
  for mode in day week month; do
    if bash "$ROOT/plugins/kai-cost/scripts/cost-report.sh" "$mode" >/dev/null 2>&1; then
...
echo "$PAYLOAD_NO_USAGE" | bash plugins/kai-cost/hooks/scripts/cost-capture.sh >/dev/null 2>&1 || true
...
[[ -f docs/plans/2026-05-10-kayoridolfi-ai-plugin-design.md ]] && t_pass "design doc presente" || t_fail "design doc ausente" ""
...
[[ -f .github/workflows/kai-ai-checks.yml ]] && t_pass "GHA workflow distribuído" || t_warn "GHA workflow ausente" ""
...
if [[ $HAVE_JQ -eq 1 ]]; then
  HOOK_GUARD="plugins/kai-security/hooks/scripts/pre-write-guard.sh"
  ...
  if [[ -x "$HOOK_GUARD" ]]; then
    ...
    # M-1: hooks.json de kai-security deve ter apenas 1 entrada Write|Edit
    HSEC="plugins/kai-security/hooks/hooks.json"
    ENTRIES=$(jq '.hooks.PreToolUse[0].hooks | length' "$HSEC")
    [[ "$ENTRIES" == "1" ]] && t_pass "kai-security hooks.json consolidado (1 entrada Write|Edit)" \
      || t_fail "kai-security hooks.json não consolidado" "esperado 1, achou $ENTRIES"

    # kai-ai-core não deve mais ter Write|Edit em PreToolUse
    HCORE="plugins/kai-ai-core/hooks/hooks.json"
    CORE_WE=$(jq '[.hooks.PreToolUse[]? | select(.matcher == "Write|Edit")] | length' "$HCORE")
    [[ "$CORE_WE" == "0" ]] && t_pass "kai-ai-core hooks.json sem Write|Edit (dedup M-1)" \
      || t_fail "kai-ai-core ainda tem Write|Edit" "achou $CORE_WE entradas"
  else
...
LDR="plugins/kai-retro/scripts/leaderboard.sh"
NWS="plugins/kai-retro/scripts/newsletter.sh"
...
if KAI_MOCK_JIRA=1 bash "$JIRA" JIRA-DEMO 2>/dev/null | jq -e '.key == "JIRA-DEMO"' >/dev/null 2>&1; then
...
SFT="plugins/kai-sdd/scripts/spec-from-ticket.sh"
...
( cd "$TMP_SFT" && git init -q && git config user.email t@t && git config user.name t && \
  KAI_MOCK_JIRA=1 bash "$REPO_PATH/$SFT" JIRA-DEMO >/dev/null 2>&1 ) && \
  [[ -f "$TMP_SFT/docs/specs/_active/$(date +%F)-jira-demo/requirements.md" ]] && \
  t_pass "/kai-spec-from-ticket gera requirements.md" || t_fail "/kai-spec-from-ticket não gerou spec" ""
rm -rf "$TMP_SFT"

# /kai-adoption-report
ADR="plugins/kai-retro/scripts/adoption-report.sh"
