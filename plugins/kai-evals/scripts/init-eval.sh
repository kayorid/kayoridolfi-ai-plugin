...
\`/kai-evals-run $FEATURE\` ou diretamente:
\`\`\`bash
...
\`/kai-evals-compare $FEATURE prompt-v1 prompt-v2\`
EOF
...
MODEL="${KAI_EVAL_MODEL:-claude-sonnet-4}"
...
echo "  4. Rode: /kai-evals-run $FEATURE"
