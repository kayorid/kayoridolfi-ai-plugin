#!/usr/bin/env bash
# kai-cost / cost-capture
# Captura uso de tokens reportado em PostToolUse.
# Não-bloqueante. Append em .kai/audit/cost.log.
...
(
  ...
  echo "$(date -u +%FT%TZ) | feature=$SAFE_FEATURE | phase=$PHASE | tool=$SAFE_TOOL | in_tokens=${IN_TOK:-0} | out_tokens=${OUT_TOK:-0} | model=$SAFE_MODEL | actor=$SAFE_ACTOR | price_in=$PRICE_IN | price_out=$PRICE_OUT | rate=$RATE" >> .kai/audit/cost.log
) 9>>.kai/audit/.cost.lock 2>/dev/null
