...
1. **Identifique fase e feature.** Se o usuário passou argumentos (`/kai-approve DESIGN onboarding-v2 "design ok"`), use-os. Caso contrário:
...
3. **Grave em `.kai/audit/approvals.log`** (append-only):
   ```
...
5. **Crie commit dedicado:**
   ```
   git add .kai/audit/approvals.log docs/specs/_active/<feature>/approvals.log
...
- Não pode aprovar fase futura pulando anteriores sem `/kai-exception`.
