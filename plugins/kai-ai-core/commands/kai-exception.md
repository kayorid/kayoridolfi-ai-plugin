---
description: Abre exceção formal a uma regra bloqueante do Kayoridolfi AI Plugin — gera issue, registra em audit-trail e aciona aprovação do maintainers
...
3. **Grave em `.kai/audit/exceptions.log`:**
   ```
...
4. **Crie issue no GitHub** (`gh issue create`):
   - Título: `[kai-exception] <regra> — <feature>`
   - Labels: `kai-exception`, `needs-maintainer-review`
...
5. **Notifique:**
   ...
   ⚠ Exceção registrada como PENDENTE
     Regra: <regra>
     Issue: <url>
     Aprovação necessária do maintainers antes da regra ser flexibilizada para você.
     Tempo estimado: 1-3 dias úteis.
   ```

6. **Não desabilite a regra automaticamente.** A flexibilização só ocorre após aprovação manual do maintainers, que então edita `.kai/config.yaml` ou allowlist do squad.
