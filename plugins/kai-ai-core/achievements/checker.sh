...
# Avalia critérios e atualiza .kai/achievements.json com novos desbloqueios.
...
STATE_FILE=".kai/achievements.json"
...
if command -v jq >/dev/null 2>&1; then
  ...
  # Force re-eval se KAI_FORCE_ACHIEVEMENT_CHECK setado (testes)
  if [[ $AGE -lt 300 && -z "${KAI_FORCE_ACHIEVEMENT_CHECK:-}" ]]; then
    exit 0  # cache válido
  fi
...
metric() {
  ...
  case "$key" in
    has_claude_md)
      [[ -f .kai/CLAUDE.md ]] && echo 1 || echo 0
      ;;
    archived_specs)
      find docs/specs/_archive -mindepth 2 -maxdepth 3 -type d 2>/dev/null | wc -l | tr -d ' '
      ;;
    blocks_secrets)
      grep -c 'pre-tool-secret-scan.*BLOCKED\|pre-tool-private-key-scan.*BLOCKED\|pre-tool-pii-scan.*BLOCKED' .kai/audit/hook-fires.log .kai/audit/security-events.log 2>/dev/null | awk -F: '{s+=$2} END {print s+0}'
      ;;
    approvals)
      [[ -f .kai/audit/approvals.log ]] && wc -l < .kai/audit/approvals.log | tr -d ' ' || echo 0
      ;;
    exceptions)
      [[ -f .kai/audit/exceptions.log ]] && wc -l < .kai/audit/exceptions.log | tr -d ' ' || echo 0
      ;;
    promoted_learnings)
      # Heurística: PRs com label proposal originados pelo squad — sem API local, contamos retros que mencionam "promote"
      grep -lr 'kai-retro-promote\|promovido ao core\|promote to core' docs/specs/_archive 2>/dev/null | wc -l | tr -d ' '
      ;;
    custom_skills)
      [[ -d .kai/skills ]] && find .kai/skills -mindepth 2 -name 'SKILL.md' 2>/dev/null | wc -l | tr -d ' ' || echo 0
      ;;
    runbooks)
      [[ -d .kai/runbooks ]] && find .kai/runbooks -name '*.md' -not -name 'README.md' 2>/dev/null | wc -l | tr -d ' ' || echo 0
      ;;
    threat_models)
    ...
    no_blocks_for_days)
      if [[ -f .kai/audit/hook-fires.log ]]; then
        local last=$(grep BLOCKED .kai/audit/hook-fires.log 2>/dev/null | tail -1 | cut -d'|' -f1 | xargs)
        if [[ -z "$last" ]]; then
          echo 999
        else
    ...
    evals_runs)
      ...
      ;;
    fast_mode_unlocked)
      [[ -f .kai/config.yaml ]] && grep -q 'modes_unlocked.*fast' .kai/config.yaml 2>/dev/null && echo 1 || echo 0
      ;;
    custom_theme)
      [[ -f .kai/config.yaml ]] && grep -E '^theme:[[:space:]]*(festive|compact|accessible)' .kai/config.yaml 2>/dev/null | head -1 | grep -q . && echo 1 || echo 0
      ;;
    compliance_checks)
