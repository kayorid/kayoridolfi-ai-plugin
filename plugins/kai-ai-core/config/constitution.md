# Constitution Kayoridolfi AI (Kayoridolfi AI Plugin)

Princípios não-negociáveis que governam todo trabalho assistido por IA no Kayoridolfi. Esta constitution é carregada em todo contexto e o agente deve respeitá-la mesmo quando instruções locais conflitarem.

**SDK em uso:** Kayoridolfi AI v0.3.1 com 9 plugins (`kai-ai-core`, `kai-bootstrap`, `kai-sdd`, `kai-review`, `kai-observability`, `kai-security`, `kai-retro`, `kai-cost`, `kai-evals`).
...
4. **Segurança não-negociável.** Hooks de segurança e compliance bloqueiam sempre — em piloto, produção, dev local, hotfix. Sem exceção via flag. Exceções formais via `/kai-exception`.

5. **Contexto vivo.** `.kai/CLAUDE.md`, glossário e skills do squad evoluem continuamente via `kai-retro`. Documentação congelada morre.
...
- MCPs externos (externos) só após avaliação de risco de exfiltração documentada.
...
- Toda fase do ciclo SDD termina com `/kai-approve <fase>` registrado.
- Pular fase exige `/kai-exception` com justificativa registrada.
- Hotfix (`/kai-hotfix`) tem post-mortem obrigatório em 48h.
- Modo `/kai-fast` só destravado para squads maduros (3+ ciclos completos, 0 exceções, 2+ learnings promovidos, 5+ achievements).
- Squads ganham achievements ao demonstrar maturidade — ver `/kai-achievements`.

### Sobre features que usam IA em runtime

- Toda feature que invoca LLM em produção (chatbot, classificador, RAG, antifraude) deve ter eval framework configurado via `kai-evals` antes de SHIP.
- Eval threshold definido junto com produto, não unilateral.
- Mudança de prompt/modelo passa por A/B compare (`/kai-evals-compare`) antes de PR.
- CI roda `/kai-evals-ci` em features AI — score abaixo do threshold bloqueia merge.

### Sobre custo de IA

- Plugin `kai-cost` captura uso de tokens automaticamente.
- Squad define orçamento mensal via `/kai-cost-budget set <valor>`.
...
**Mudanças:** via PR ao `kayoridolfi-ai-plugin` com aprovação do maintainers + 2 AI Champions.
