#!/usr/bin/env bash
# kai-ai-core / SessionStart banner
# Mostra banner  + frase do manifesto + status rápido.
...
if [[ -f .kai/CLAUDE.md ]]; then
  BOOTSTRAPPED="✓"
  SQUAD=$(grep -m1 '^# CLAUDE.md' .kai/CLAUDE.md 2>/dev/null | sed 's/^# CLAUDE.md — //' || echo "squad")
fi
...
${C_GREEN}▸${C_RESET} Comece com ${C_BOLD}/kai-help${C_RESET} para ver tudo que o SDK oferece.
${C_GREEN}▸${C_RESET} ${C_BOLD}/kai-status${C_RESET} para diagnóstico do repo.
$([[ "$BOOTSTRAPPED" == "✗" ]] && echo "${C_GREEN}▸${C_RESET} ${C_BOLD}/kai-bootstrap${C_RESET} para configurar o squad pela primeira vez.")
EOF
)
...
PLAIN_CTX="Kayoridolfi AI Plugin ativo · Squad: ${SQUAD} · Bootstrap: ${BOOTSTRAPPED} · Specs ativas: ${ACTIVE_SPECS} · Fase atual: ${PHASE}

Constitution corporativa  carregada via kai-ai-core. Hooks bloqueantes ativos. Use /kai-help para visão geral, /kai-status para diagnóstico, /kai-doctor para health check.${NEW_ACHS:+
