...
if [[ -f .kai/CLAUDE.md ]]; then
  ...
  SQUAD_RAW=$(grep -m1 '^# CLAUDE.md' .kai/CLAUDE.md 2>/dev/null | sed 's/^# CLAUDE.md — //' || echo "squad")
...
if [[ -f .kai/audit/hook-fires.log ]]; then
  TODAY=$(date -u +%Y-%m-%d)
  BLOCKS=$(grep -c "^${TODAY}.*BLOCKED" .kai/audit/hook-fires.log 2>/dev/null || echo 0)
fi
...
if [[ $COLS -lt 80 ]]; then
  # Mínimo: só  + fase + blocks
  printf "${C_GOLD}◆ KAI${C_RESET} ${C_DIM}·${C_RESET} ${C_ORANGE}%s${C_RESET} ${C_DIM}·${C_RESET} ${C_DIM}🛡 %d${C_RESET}" "$PHASE" "$BLOCKS"
elif [[ $COLS -lt 100 ]]; then
  # Compacto: sem nome de model
  printf "${C_GOLD}◆ KAI${C_RESET} ${C_DIM}·${C_RESET} ${C_CYAN}%s${C_RESET} ${C_DIM}·${C_RESET} %s ${C_DIM}·${C_RESET} %d ${C_DIM}·${C_RESET} ${C_ORANGE}%s${C_RESET} ${C_DIM}·${C_RESET} 🛡 %d" \
    "$SQUAD" "$BOOT" "$ACTIVE" "$PHASE" "$BLOCKS"
else
  # Formato completo
  printf "${C_GOLD}◆ KAI${C_RESET} ${C_DIM}│${C_RESET} ${C_CYAN}%s${C_RESET} ${C_DIM}│${C_RESET} ${C_DIM}boot${C_RESET} %s ${C_DIM}│${C_RESET} ${C_DIM}specs${C_RESET} %d ${C_DIM}│${C_RESET} ${C_ORANGE}%s${C_RESET} ${C_DIM}│${C_RESET} ${C_DIM}🛡${C_RESET} %d ${C_DIM}│${C_RESET} ${C_DIM}%s${C_RESET}" \
