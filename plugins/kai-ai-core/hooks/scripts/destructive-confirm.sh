#!/usr/bin/env bash
# kai-ai-core / pre-bash destructive confirm
...
for PAT in "${DANGER[@]}"; do
  if echo "$CMD" | grep -Eiq -e "$PAT"; then
    ...
    if echo "$CMD" | grep -Eq '#[[:space:]]*KAI_CONFIRMED'; then
      AUDIT_DIR=".kai/audit"
      mkdir -p "$AUDIT_DIR"
    ...
    cat <<EOF >&2
[kai-ai-core] BLOCKED — comando destrutivo detectado
  Comando: $(echo "$CMD" | head -c 200)
  Padrão:  $PAT
  Para confirmar intenção, anexe \`# KAI_CONFIRMED\` ao final do comando, ou execute manualmente fora do agente.
