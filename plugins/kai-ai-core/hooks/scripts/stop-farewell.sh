...
if [[ -f .kai/audit/approvals.log ]]; then
  APPROVALS_TODAY=$(grep -c "^${TODAY}" .kai/audit/approvals.log 2>/dev/null || echo 0)
fi
if [[ -f .kai/audit/hook-fires.log ]]; then
  BLOCKS_TODAY=$(grep -c "^${TODAY}.*BLOCKED" .kai/audit/hook-fires.log 2>/dev/null || echo 0)
fi
...
FAREWELLS=(
  ...
  "Construindo o  de 2027, um spec por vez."
)
HASH=$(date +%j)
IDX=$(( HASH % ${#FAREWELLS[@]} ))
QUOTE="${FAREWELLS[$IDX]}"
...
if command -v jq >/dev/null 2>&1; then
  PLAIN_FAREWELL="Kayoridolfi AI · Sessão encerrada · ${APPROVALS_TODAY} aprovação(ões) hoje · ${BLOCKS_TODAY} bloqueio(s) defensivo(s) · ${QUOTE}"
