...
# Métricas
SQUAD="∅"
[[ -f .kai/CLAUDE.md ]] && SQUAD=$(grep -m1 '^# CLAUDE.md' .kai/CLAUDE.md 2>/dev/null | sed 's/^# CLAUDE.md — //' | head -c 30)
...
APPROVALS=$([ -f .kai/audit/approvals.log ] && wc -l < .kai/audit/approvals.log | tr -d ' ' || echo 0)
EXCEPTIONS=$([ -f .kai/audit/exceptions.log ] && wc -l < .kai/audit/exceptions.log | tr -d ' ' || echo 0)

BLOCKS_WEEK="n/d"
if [[ -f .kai/audit/hook-fires.log ]]; then
  WEEK_AGO=$(date -u -v-7d +%Y-%m-%d 2>/dev/null || date -u -d "7 days ago" +%Y-%m-%d 2>/dev/null || echo "")
  if [[ -n "$WEEK_AGO" ]]; then
    BLOCKS_WEEK=$(awk -v wa="$WEEK_AGO" -F'|' '$1>=wa && /BLOCKED/' .kai/audit/hook-fires.log 2>/dev/null | wc -l | tr -d ' ')
  fi
fi
...
ACH_TOTAL=16
ACH_UNLOCKED=0
if [[ -f .kai/achievements.json ]] && command -v jq >/dev/null 2>&1; then
  ACH_UNLOCKED=$(jq -r '.unlocked | length' .kai/achievements.json 2>/dev/null || echo 0)
fi

# kai-evals: contagem de evals + última run
...
# kai-cost: custo do mês corrente (última linha summary se houver)
COST_MONTH="—"
if [[ -f .kai/audit/cost.log ]]; then
  COST_LINES=$(wc -l < .kai/audit/cost.log | tr -d ' ')
  COST_MONTH="${COST_LINES} eventos"
fi

# Modo /kai-fast
FAST="—"
[[ -f .kai/config.yaml ]] && grep -q 'modes_unlocked.*fast' .kai/config.yaml 2>/dev/null && FAST="✓ destravado"
...
[[ -f .kai/skills/README.md ]] && MATURITY=$((MATURITY+15))
...
NEXT="—"
if [[ ! -f .kai/CLAUDE.md ]]; then
  NEXT="/kai-bootstrap (squad ainda não bootstrapado)"
elif [[ $ACTIVE -eq 0 ]]; then
  NEXT="/kai-spec <slug> para iniciar próxima feature"
else
  LATEST=$(find docs/specs/_active -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort | tail -1)
  if [[ -n "$LATEST" ]]; then
    SLUG=$(basename "$LATEST" | sed 's/^[0-9-]*//')
    if [[ ! -f "$LATEST/discuss.md" ]]; then NEXT="/kai-spec-discuss $SLUG"
    elif [[ ! -f "$LATEST/requirements.md" ]]; then NEXT="/kai-spec-requirements $SLUG"
    elif [[ ! -f "$LATEST/design.md" ]]; then NEXT="/kai-spec-design $SLUG"
    elif [[ ! -f "$LATEST/tasks.md" ]]; then NEXT="/kai-spec-plan $SLUG"
    elif [[ ! -f "$LATEST/verification.md" ]]; then NEXT="/kai-spec-execute $SLUG ou /kai-spec-verify $SLUG"
    elif [[ ! -f "$LATEST/REVIEW.md" ]]; then NEXT="/kai-review-pr para spec $SLUG"
    elif [[ ! -f "$LATEST/retro.md" ]]; then NEXT="/kai-spec-retro $SLUG"
    fi
  fi
fi
...
${P}┌─ Kayoridolfi AI Plugin · Squad ${B}${SQUAD}${R}${P} · ${Y}-Q${Q} ──────────────────────${R}
${P}│${R}
...
${P}│${R}  ${I}Achievements:${R}  ${B}${ACH_UNLOCKED}/${ACH_TOTAL}${R}      ${I}Modo /kai-fast:${R} ${B}${FAST}${R}
${P}│${R}
