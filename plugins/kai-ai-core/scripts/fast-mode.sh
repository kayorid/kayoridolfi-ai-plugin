...
# Verifica critérios de maturidade e destrava /kai-fast
...
CONFIG=".kai/config.yaml"
[[ ! -d .kai ]] && { printf "${C_DANGER:-}✗${C_RESET:-} Squad não bootstrapado. Rode /kai-bootstrap primeiro.\n"; exit 1; }
...
PROMOTED=$(grep -lr 'kai-retro-promote\|promovido ao core' docs/specs/_archive 2>/dev/null | wc -l | tr -d ' ')
ACH_UNLOCKED=0
[[ -f .kai/achievements.json ]] && command -v jq >/dev/null && \
  ACH_UNLOCKED=$(jq -r '.unlocked | length' .kai/achievements.json 2>/dev/null || echo 0)

printf "\n${C_PRIMARY:-}${C_BOLD:-}Kayoridolfi AI · Verificação de maturidade${C_RESET:-}\n\n"
...
if [[ $OK -eq $TOTAL ]]; then
  ...
  printf "\n${C_SUCCESS:-}${C_BOLD:-}🏆 SQUAD MADURO — /kai-fast destravado.${C_RESET:-}\n"
  printf "${C_DIM:-}Tarefas pequenas (< 1 dia) podem usar /kai-fast direto. Features continuam pelo ciclo SDD completo.${C_RESET:-}\n\n"
else
