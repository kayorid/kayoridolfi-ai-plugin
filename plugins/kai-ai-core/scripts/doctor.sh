...
# Diagnóstico completo do Kayoridolfi AI Plugin no ambiente atual.
...
kai_load_glyphs 2>/dev/null || true
...
printf "${C_PRIMARY:-}${C_BOLD:-}\n  Kayoridolfi AI Plugin · Diagnóstico\n${C_RESET:-}\n"
...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  if [[ -n "$AUTHOR" ]]; then
    ...
    warn "git user.email não configurada" "Necessária para /kai-approve gravar autor"
  fi
else
...
sec "Estrutura .kai/"
if [[ -d .kai ]]; then
  ok "Diretório .kai/ existe"
  for f in CLAUDE.md glossary.md; do
    if [[ -f .kai/$f ]]; then ok ".kai/$f presente"; else warn ".kai/$f ausente" "Rode /kai-bootstrap ou /kai-bootstrap-rescan"; fi
  done
  for d in audit runbooks skills hooks bootstrap; do
    if [[ -d .kai/$d ]]; then ok ".kai/$d/ presente"; else warn ".kai/$d/ ausente" "Será criado quando necessário"; fi
  done
  if [[ -f .kai/config.yaml ]]; then ok ".kai/config.yaml presente"; else warn ".kai/config.yaml ausente" "Configuração padrão será assumida"; fi
else
  warn "Diretório .kai/ ausente" "Squad ainda não bootstrapado — rode /kai-bootstrap"
fi

sec "Plugins KAI"
PLUGIN_PARENT=$(dirname "$PLUGIN_ROOT")
EXPECTED=(kai-ai-core kai-bootstrap kai-sdd kai-review kai-observability kai-security kai-retro kai-cost kai-evals)
...
if [[ -d .kai/audit ]]; then
  for log in approvals.log exceptions.log hook-fires.log security-events.log commands.log; do
    if [[ -f .kai/audit/$log ]]; then
      LINES=$(wc -l < ".kai/audit/$log" | tr -d ' ')
      ok "$log ($LINES linha(s))"
    fi
  done
else
  warn ".kai/audit ausente" "Será criado no primeiro uso"
fi
