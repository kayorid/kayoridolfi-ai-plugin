...
# Verifica e aplica atualizações do Kayoridolfi AI SDK.
...
CACHE="${HOME}/.claude/plugins/cache/kai"
...
printf "\n${C_PRIMARY:-}${C_BOLD:-}Kayoridolfi AI · Atualização${C_RESET:-}\n\n"
...
LATEST=$(git -C "$CACHE" show origin/main:.claude-plugin/marketplace.json 2>/dev/null | jq -r '.plugins[] | select(.name == "kai-ai-core") | .version' || echo "$CURRENT")
