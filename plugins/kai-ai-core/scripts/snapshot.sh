...
# Snapshot reversível de .kai/ e docs/specs/_active/
...
case "$SUB" in
  create)
    ...
    if [[ ${#TARGETS[@]} -eq 0 ]]; then
      printf "${C_WARN:-}${G_WARN:-⚠}${C_RESET:-} Nada a snapshotar (.kai e docs/specs/_active ausentes)\n"
      exit 0
    fi

    # exclui o próprio diretório de snapshots
    tar --exclude=".kai/_snapshots" -czf "$OUT" "${TARGETS[@]}" 2>/dev/null
  ...
  restore)
    ...
    if [[ ${#TARGETS[@]} -gt 0 ]]; then
      tar --exclude=".kai/_snapshots" -czf "$BACKUP" "${TARGETS[@]}" 2>/dev/null
  ...
  help|*)
    cat <<EOF
Kayoridolfi Snapshot — backup reversível de .kai/ e specs ativas

Uso:
  /kai-snapshot create [tag]     Cria snapshot opcional com tag
  /kai-snapshot list             Lista snapshots disponíveis
  /kai-snapshot restore <nome>   Restaura snapshot (faz backup do atual antes)

Snapshots ficam em .kai/_snapshots/ (ignorado em git via .gitignore se desejado).
EOF
    ;;
esac
