...
# Scaffolder de skill custom em .kai/skills/
...
# Atualiza index
INDEX=".kai/skills/README.md"
[[ ! -f "$INDEX" ]] && echo "# Skills custom do squad" > "$INDEX"
echo "" >> "$INDEX"
echo "- [kai-${NAME}](kai-${NAME}/SKILL.md) — <descrição curta>" >> "$INDEX"
...
printf "\n${C_SUCCESS:-}✓${C_RESET:-} Skill ${C_BOLD:-}kai-${NAME}${C_RESET:-} criada em ${DIR}\n\n"
...
printf "  5. Commit: ${C_BOLD:-}git commit -m \"[skills] cria skill custom kai-${NAME}\"${C_RESET:-}\n\n"
