---
name: kai-observability-designer
...
|--------|----------|-----------|---------|
| order_error_rate_high | rate(errors)/rate(total) > 5% por 5min | P1 | .kai/runbooks/orders-error.md |
...
- Para cada alerta, crie ou referencie um runbook (chame `/kai-runbook-from-incident` se aplicável).
