#!/usr/bin/env bash
# kai-retro / adoption-report.sh
# Relatório de adoção corporativa do Kayoridolfi AI Plugin.
...
# Retros recentes
RETROS_RECENT=$({
  find .kai/retros -name '*.md' -type f -newer /tmp -mtime -90 2>/dev/null
  find docs/specs/_archive -name 'retro.md' -type f 2>/dev/null
} | wc -l | tr -d ' ')

# Plugins ativos (heurística por hooks executados em .kai/audit/hook-fires.log)
if [[ -f .kai/audit/hook-fires.log ]]; then
  PLUGINS_USED=$(grep -oE 'hook=[a-z-]+' .kai/audit/hook-fires.log 2>/dev/null \
    | sort -u | head -20)
fi
...
printf "\n${P}${B}Kayoridolfi AI · Adoption Report${R} ${D}— últimas $PERIOD${R}\n"
printf "${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${R}\n\n"
