...
# Encontra retros: .kai/retros/, docs/specs/_archive/, docs/specs/_completed/
RETROS_TMP=$(mktemp)
{
...
# ----- Markdown -----
cat > "$MD" <<EOF
# Kayoridolfi AI Newsletter — ${QUARTER}
...
<head>
  <meta charset="utf-8">
  <title>Kayoridolfi AI Newsletter — ${QUARTER}</title>
</head>
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;max-width:680px;margin:32px auto;color:#1a1a1a;line-height:1.55;padding:0 16px;">
  <header style="border-bottom:3px solid #E8550C;padding-bottom:16px;margin-bottom:24px;">
    <h1 style="margin:0;color:#E8550C;font-size:28px;">Kayoridolfi AI Newsletter</h1>
  ...
  <footer style="margin-top:32px;padding-top:16px;border-top:1px solid #eee;color:#888;font-size:13px;">
    Gerado por <code>plugins/kai-retro/scripts/newsletter.sh</code>. Para customizar, edite o template no script ou contribua via PR.
  </footer>
</body>
</html>
EOF
