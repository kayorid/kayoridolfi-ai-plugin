...
source "$PLUGIN_ROOT/../kai-ai-core/lib/colors.sh" 2>/dev/null || true
...
printf "\n${P}${B}Kayoridolfi AI · Leaderboard${R} ${D}— últimas $PERIOD, agregado por squad${R}\n"
printf "${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${R}\n\n"
...
if [[ -d .kai/retros ]] || [[ -d docs/specs/_archive ]] || [[ -d docs/specs/_completed ]]; then
  {
    find .kai/retros -name '*.md' -type f 2>/dev/null
...
if [[ -f .kai/achievements.json ]]; then
  jq -r '.unlocked[]? | "  ✦ \(.icon // "★") \(.name) (\(.unlocked_at | .[:10]))"' .kai/achievements.json 2>/dev/null \
    | head -5
  TOTAL=$(jq -r '.unlocked | length' .kai/achievements.json 2>/dev/null || echo 0)
