#!/usr/bin/env bash
# kai-security / pre-tool PII scan
...
if echo "$CONTENT" | grep -Eq '\b[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}\b'; then
  ...
  if [[ "$HIT" != "000.000.000-00" && "$HIT" != "111.111.111-11" ]]; then
    ...
    cat <<EOF >&2
[kai-security] BLOCKED — possível CPF detectado
  Arquivo: $FILE_PATH
  Padrão:  $HIT
  Para fixtures: marque o arquivo com sufixo _test, _fixture ou similar.
  Para necessidade legítima: /kai-exception
EOF
    exit 2
  fi
fi
...
if echo "$CONTENT" | grep -Eq '\b[0-9]{2}\.[0-9]{3}\.[0-9]{3}/[0-9]{4}-[0-9]{2}\b'; then
  ...
  echo "[kai-security] BLOCKED — CNPJ detectado em $FILE_PATH ($HIT). Use placeholder ou marque arquivo como fixture." >&2
  exit 2
fi
...
if echo "$CONTENT" | grep -Eq -e '\b(4[0-9]{12}([0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(011|5[0-9]{2})[0-9]{12})\b'; then
  ...
  echo "[kai-security] BLOCKED — número de cartão detectado em $FILE_PATH. Nunca commitar cartões reais." >&2
  exit 2
fi
