#!/usr/bin/env bash
# kai-security / pre-tool private key scan
...
for PAT in "${PATTERNS[@]}"; do
  if printf '%s' "$CONTENT" | grep -Eiq -e "$PAT"; then
    ...
    cat <<EOF >&2
[kai-security] BLOCKED — possível chave privada detectada
  Arquivo: $FILE_PATH
  Tipo:    $PAT
  Chaves privadas NUNCA podem ser commitadas, nem em arquivos de teste.
  Use HSM/KMS para gestão.
  Esta regra é não-negociável e não aceita exceção via /kai-exception.
EOF
    exit 2
  fi
done
