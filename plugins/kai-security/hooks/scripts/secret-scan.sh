#!/usr/bin/env bash
# kai-ai-core / pre-tool secret scan
...
for PAT in "${PATTERNS[@]}"; do
  if echo "$CONTENT" | grep -Eq -e "$PAT"; then
    ...
    cat <<EOF >&2
[kai-ai-core] BLOCKED — segredo detectado
  Arquivo: $PATH_FIELD
  Padrão:  $PAT
  Ação:    remova o segredo ou use placeholder (<YOUR-KEY-HERE>).
  Se for falso-positivo, abra exceção formal: /kai-exception
EOF
    exit 2
  fi
done
...
if [[ "$PATH_FIELD" == *.env && $ENV_ALLOW -eq 0 ]]; then
  ...
  if [[ -n "$REAL_VALUES" ]]; then
    ...
    cat <<EOF >&2
[kai-ai-core] BLOCKED — arquivo .env com valores reais
  Arquivo: $PATH_FIELD
