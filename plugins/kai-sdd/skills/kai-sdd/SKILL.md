---
name: kai-sdd
description: Use sempre que o usuário estiver planejando uma fase do projeto, iniciando uma nova feature, no meio de uma implementação não trivial, ou finalizando uma entrega. Acione também ao mencionar "spec", "especificação", "requirements", "kickoff", "planejamento", "handover", "retrospectiva", ou pedir para "documentar o que vamos fazer/fizemos". Spec-Driven Development do Kayoridolfi AI Plugin — fluxo rígido com checkpoints obrigatórios, audit-trail via /kai-approve e integração com kai-ai-core, kai-review, kai-security, kai-observability e kai-retro. Cobre o ciclo end-to-end (discuss → spec → plan → execute → verify → review → ship → retro). Acione mesmo quando o usuário não pedir explicitamente "spec" — qualquer trabalho não trivial multi-passo se beneficia desta skill.
---

#  Spec-Driven Development (kai-sdd)

Esta skill é a espinha dorsal do Kayoridolfi AI Plugin. Transforma desenvolvimento assistido por IA em um processo disciplinado por especificações vivas, com checkpoints humanos obrigatórios e audit-trail compatível com requisitos regulatórios (Bacen, CVM, LGPD).

## Integração com o ecossistema

- **Constitution corporativa** carregada via `kai-ai-core` (skill `kai-constitution`).
- **Checkpoints registrados** via `/kai-approve <fase>` em `.kai/audit/approvals.log`.
- **Threat-model obrigatório** (via `kai-security`) para features tocando ativos críticos.
- **Observability design** (via `kai-observability`) integrado à fase PLAN.
- **Review automático** (via `kai-review`) na fase SHIP.
- **Retro estruturada** (via `kai-retro`) com promoção de learnings ao core.
