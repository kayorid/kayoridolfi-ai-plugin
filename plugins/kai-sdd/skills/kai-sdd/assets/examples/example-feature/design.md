...
CSV é montado em memória durante a geração; para tenants > 1000 colaboradores, usamos streaming de chunks para evitar pico de RAM. Limite operacional inicial: 50  por arquivo (cobre tenants até ~500k linhas).
...
| S3 abstraction | Storage temporário | ~$0.01/GB/mês | 50  por export |
...
### ⚠️ Ask first
- Aumentar limite de 50  → impacto em custos S3 e timeout do worker
