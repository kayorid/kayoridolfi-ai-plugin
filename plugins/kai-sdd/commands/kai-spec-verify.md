...
9. **Para features que usam IA em runtime** (chatbot, RAG, classificador, antifraude):
   ...
   - Se não há, oriente criar via `/kai-evals-init <feature>` antes de prosseguir.
   - Rode `/kai-evals-ci <feature>` — score abaixo do threshold bloqueia VERIFY.
   - Anexe link da run em `verification.md`.
10. Se tudo PASS (incluindo evals quando aplicável), peça `/kai-approve VERIFY` e oriente seguir para REVIEW.
