...
0. **Banner de hotfix:**
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/../kai-ai-core/lib/ascii.sh" hotfix danger
   ```
1. **Validação inicial:**
   - Pergunte: "Qual o incidente/urgência? Há ticket de incidente associado?"
   - Se a justificativa for fraca (não-urgência), oriente usar `/kai-spec` normal.
2. **Setup:**
...
6. **Post-mortem obrigatório em 48h:**
   ...
   - Conduza com `/kai-spec-retro` adaptado: causa raiz formal, ações para evitar recorrência, learnings.
   - Ações de prevenção viram tasks formais em outra spec.

## Diferenças vs `/kai-spec`

| Fase | `/kai-spec` | `/kai-hotfix` |
|------|-----------|--------------|
