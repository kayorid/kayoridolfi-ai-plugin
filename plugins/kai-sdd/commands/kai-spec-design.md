...
2. Leia `requirements.md`, `.kai/CLAUDE.md`, glossário.
3. Verifique se a feature toca ativo crítico (regulação, fluxos críticos do squad). Se sim, **invoque `/kai-threat-model` antes de prosseguir** e referencie `THREAT-MODEL.md` no design.
...
6. Invoque `/kai-observability-design` para gerar `OBSERVABILITY.md` paralelo.
7. Peça `/kai-approve DESIGN`.
