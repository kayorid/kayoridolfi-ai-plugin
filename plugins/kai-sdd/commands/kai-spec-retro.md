...
0. **Banner de retro:**
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/../kai-ai-core/lib/ascii.sh" retro accent
   ```
1. Verifique SHIP completo (PR mergeado).
2. Invoque a skill `kai-retro-facilitator` (do plugin `kai-retro`) para conduzir o fluxo. Se `kai-retro` não estiver instalado, conduza inline:
...
4. Identifique se há learning promovível à constitution (`kai-ai-core`):
   - Se sim, oriente abrir PR ao `kayoridolfi-ai-plugin` propondo a mudança.
5. Peça `/kai-approve RETRO`.
